...
trigger_build_url=$(cat Utils/gitlab_triggers/trigger_build_url.txt)

curl "$trigger_build_url" -F "ref=$_branch" -F "token=$_gitlab_token" -F "variables[NIGHTLY]=true" -F "variables[IFRA_ENV_TYPE]=Nightly" | jq
